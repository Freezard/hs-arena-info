...
let HSArenaInfo = (function() {
	...
	*********************************************************/
    ...
    // Filter when browsing all arena cards
    let filter = {
		cost: '',
		type: '',
        tribe: '',
	...
	*********************************************************/
    ...
    function createStatsMenu(type) {
        ...
        for (let cost in totalStats) {
            ...
            let img = document.createElement('img');
            img.setAttribute('src', 'attack.png');
            if (type !== 'AVERAGE')
                div2.innerHTML = ((totalStats[cost] / totalMinions[cost]) * 100).toFixed(2) + '%';
            else div2.innerHTML = '<img src="attack.png">' + (totalStats[cost].attack / totalMinions[cost]).toFixed(2) +
                    ' <img src="health.png">' + (totalStats[cost].health / totalMinions[cost]).toFixed(2);
